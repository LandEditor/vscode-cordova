...
import {
	...
	TargetType,
	WebviewData,
} from "../../debugger/cordovaDebugSession";
...
> {
	...
	public async launchApp(): Promise<IIosLaunchResult> {
		...
		if (!this._target.isVirtualTarget) {
			await CordovaIosDeviceLauncher.startDebugProxy(
	...
	public async prepareForAttach(): Promise<IIosAttachResult> {
		...
		return retryAsync<IIosAttachResult>(
			...
			ErrorHelper.getInternalError(InternalErrorCode.UnableToFindWebview)
				.message,
	...
	protected async getFirstAvailableOnlineTarget(): Promise<IOSTarget> {
		return new IOSTarget(
			(await this.getFirstDebuggableTarget()) as IDebuggableIOSTarget,
		);
	}
