...
> {
	...
	public async prepareForAttach(): Promise<IAndroidAttachResult> {
		...
		const findAbstractNameFunction = () => {
			return this.adbHelper.getDevToolsAbstractName(
				target.id,
				appPackageName,
			);
		};
		...
		const abstractName = await retryAsync(
			...
			ErrorHelper.getInternalError(
				InternalErrorCode.UnableToFindLocalAbstractName,
			).message,
	...
	public getRunArguments(): string[] {
		...
		if (
			...
			this.platformOpts.runArguments.length > 0
		) {
			args.push(...this.platformOpts.runArguments);
		} else {
			if (this.platformOpts.target) {
				args.push(
					AdbHelper.isVirtualTarget(this.platformOpts.target)
						? "--emulator"
						: "--device",
				);
	...
	public async getTargetFromRunArgs(): Promise<AndroidTarget | undefined> {
		if (
			this.platformOpts.runArguments &&
			this.platformOpts.runArguments.length
		) {
	...
	private async getAppPackageName(): Promise<string> {
		...
		try {
			manifestContents = await fs.promises.readFile(
				path.join(
					this.projectRoot,
					AndroidPlatform.ANDROID_MANIFEST_PATH,
				),
			);
		} catch (error) {
			if (error && error.code === "ENOENT") {
				manifestContents = await fs.promises.readFile(
					path.join(
						this.projectRoot,
						AndroidPlatform.ANDROID_MANIFEST_PATH_8,
					),
				);
			} else {
				throw error;
