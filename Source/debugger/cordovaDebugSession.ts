...
export default class CordovaDebugSession extends LoggingDebugSession {
	...
	constructor(
		...
		if (
			...
			!SimulateHelper.isSimulate({
				...
				simulatePort:
					this.vsCodeDebugSession.configuration.simulatePort,
			})
		) {
		...
		const doAttach = async (attachArgs: ICordovaAttachRequestArgs) => {
			try {
				...
				await TelemetryHelper.generate("attach", async (generator) => {
					TelemetryHelper.sendPluginsList(
						attachArgs.cwd,
						CordovaProjectHelper.getInstalledPlugins(
							attachArgs.cwd,
						),
					);
					generator.add(
		...
		if (this.cordovaCdpProxy) {
			if (attachArgs.target == TargetType.Electron) {
				const attachArguments =
					this.jsDebugConfigAdapter.createElectronDebuggingConfig(
						attachArgs,
						CordovaDebugSession.CDP_PROXY_PORT,
						this.pwaSessionName,
						this.cordovaSession.getSessionId(),
	...
	private async resolvePlatform(
		args: ICordovaAttachRequestArgs &
			Partial<
				Omit<ICordovaLaunchRequestArgs, keyof ICordovaAttachRequestArgs>
			>,
		...
		if (target == TargetType.Chrome) {
			userDataPath = path.join(
				settingsHome(),
				BrowserPlatform.CHROME_DATA_DIR,
			);
		} else if (target == TargetType.Edge) {
			userDataPath = path.join(
				settingsHome(),
				BrowserPlatform.EDGE_DATA_DIR,
			);
		} else {
			userDataPath = "";
		}
	...
	private async changeSimulateViewport(
	...
	private async terminate(): Promise<void> {
		await vscode.commands.executeCommand(
			CordovaDebugSession.STOP_COMMAND,
			undefined,
			{
