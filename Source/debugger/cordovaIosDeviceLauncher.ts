...
export class CordovaIosDeviceLauncher {
	...
	public static startWebkitDebugProxy(
		...
		return new Promise(async (resolve, reject) => {
			...
			CordovaIosDeviceLauncher.webDebuggerProxyInstance =
	...
	public static async getPathOnSimulator(
		...
		throw ErrorHelper.getInternalError(
			InternalErrorCode.ApplicationNotInstalledOnTheSimulator,
		);
	}
	...
	private static mountDeveloperImage(): Promise<void> {
		return CordovaIosDeviceLauncher.getDiskImage().then((path: string) => {
			const imagemounter: child_process.ChildProcess =
				child_process.spawn(
					"ideviceimagemounter",
					[path, `${path}.signature`],
					{ shell: true },
	...
	private static getDiskImage(): Promise<string> {
		...
		return Promise.all([versionInfo, pathInfo]).then(
			([version, sdkpath]) => {
