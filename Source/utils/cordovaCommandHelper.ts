...
export class CordovaCommandHelper {
	...
	public static executeCordovaCommand(
		...
		return CordovaCommandHelper.selectPlatform(
			...
			TelemetryHelper.generate(telemetryEventName, (generator) => {
				...
				const runArgs =
					CordovaCommandHelper.getRunArguments(projectRoot);
				if (runArgs.length) {
				...
				const execution = new Promise((resolve, reject) => {
					...
					if (platform === "android") {
						process =
							CordovaAndroidEnvironmentHelper.checkEnvironment(
								projectRoot,
								env,
								logger,
							)
	...
	public static restartCordovaDebugging(
		...
		const cordovaDebugSession =
			cordovaSessionManager.getCordovaDebugSessionByProjectRoot(
				projectRoot,
			);
			...
			switch (cordovaDebugSession.getStatus()) {
				case CordovaSessionStatus.Activated:
					...
					commands.executeCommand(
						...
						{
							sessionId:
								cordovaDebugSession.getVSCodeDebugSession().id,
						},
					);
					break;
	...
	public static getCordovaExecutable(fsPath: string): string {
		return (
			CordovaCommandHelper.getSetting(fsPath, "cordovaExecutable") || ""
		);
	}
