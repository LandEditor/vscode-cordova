...
export class CordovaProjectHelper {
	...
	public static getInstalledPluginDetails(
		...
		try {
			...
			const details: IPluginDetails = {
				...
				PluginType: CordovaProjectHelper.CORE_PLUGIN_LIST.includes(
					pluginId,
				)
					? "Core"
					: "Npm",
	...
	public static isCordovaProject(workspaceRoot: string): boolean {
		return !!(
			CordovaProjectHelper.existsSync(
				path.join(
					workspaceRoot,
					CordovaProjectHelper.CONFIG_XML_FILENAME,
				),
			) ||
			CordovaProjectHelper.existsSync(
				path.join(
					workspaceRoot,
					CordovaProjectHelper.CONFIG_IONIC_FILENAME,
				),
			)
		);
	}
	...
	public static determineIonicMajorVersion(
		...
		if (
			...
			fs.existsSync(
				path.join(
					projectRoot,
					CordovaProjectHelper.IONIC_LIB_DEFAULT_PATH,
				),
			)
		) {
			return 1;
	...
	public static isIonicCliVersionGte3(
		...
		return CordovaProjectHelper.isIonicCliVersionGte(
			fsPath,
			"3.0.0",
			command,
		);
	...
	private static versionSatisfiesInterval(
		...
		return (
			semver.gt(version, lowVersionEdge) &&
			semver.lt(version, highVersionEdge)
		);
	}
	...
	public static getCordovaPackageName(projectRoot: string): string {
		const packageName = JSON.parse(
			fs.readFileSync(
				findFileInFolderHierarchy(projectRoot, "package.json"),
				"utf-8",
			),
		).name;
		return packageName;
